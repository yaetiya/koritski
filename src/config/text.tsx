//EN LANG

export const enLang = {
  headl: {
    projects: "PROJECTS",
    about: "ABOUT US",
    form: "TELL US ABOUT YOUR APPLICATION IDEA",
  },
  header: {
    orderBtn: "ORDER",
    slog: "KNOW YOUR DIGITAL INTELLIGENCE",
  },
  projects: {
    cards: {
      0: {
        h: "Web Site",
        description:
          "Sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo. Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui rati",
      },
      1: {
        h: "Mobile app",
        description:
          "Sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo. Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui rati",
      },
    },
  },
  form: {
    alerts: {
      error: "Your data is not correct",
      submit: "Submitted!",
    },
    submitBtn: "SUBMIT",
    placeholders: {
      phone: "PHONE",
      email: "EMAIL",
      company: "COMPANY",
      idea: "DESCRIBE YOUR IDEA",
    },
  },
};

export const enSlider = [
  {
    label: "Founder of koritski.com. Brings to life most profound ideas of yours",
    h1: "Founder",
    h2: "Developer in chief",
    name: "Alexander Koritskiy",
  },
  {
    label: "Main technical supporter and the beauty of the team in any possible aspect",
    h1: "Senior developer",
    h2: "Back end",
    name: "Maria Astashkina",
  },
  {
    label: "Main representative of the studio. In charge of any executive issues",
    h1: "Business manager",
    h2: "CEO",
    name: "Maxim Evenko",
  },
  {
    label: "The creative power in the office. Seizes your style and draws visual appearance of the product. ",
    h1: "Designer, front-end developer",
    h2: "Mobile developer",
    name: "Mikhail Egorov",
  },
];

//RU LANG
export const ruLang = {
  headl: {
    projects: "ПРОЕКТЫ",
    about: "О НАС",
    form: "РАССКАЖИТЕ НАМ О ВАШЕЙ ИДЕЕ",
  },
  header: {
    orderBtn: "ЗАКАЗАТЬ",
    slog: "ОТКРОЙ СВОИ ВОЗМОЖНОСТИ",
  },
  projects: {
    cards: {
      0: {
        h: "Веб Сайт",
        description: "Вэб-сайты с широким функционалом, которые станут вашим самым покорным партнером в интернет пространстве: от одностраничных лэндингов до интернет-магазинов с тысячами товаров. Мы создаем онлайн лицо вашего бизнеса – первое впечатление от вас при знакомстве с клиентом. ",
      },
      1: {
        h: "Мобильное приложение",
        description: "Мобильные приложения, переносящие вас, вашу команду, продукт и клиентов на расстояние вытянутой руки. Самый простой способ навсегда остаться с вашим клиентом или быть на связи с вашими сотрудниками. Мы стираем проблему места и времени из всего процесса CR и менеджмента.",
      },
    },
  },
  form: {
    alerts: {
      error: "Вы ввели некорректные данные",
      submit: "Отправлено!",
    },
    submitBtn: "ОТПРАВИТЬ",
    placeholders: {
      phone: "ТЕЛЕФОН",
      email: "ПОЧТА",
      company: "КОМПАНИЯ",
      idea: "ОПИШИТЕ ВАШУ ИДЕЮ",
    },
  },
};

export const ruSlider = [
  {
    label: "Основатель проекта koritski.com. Воплощает в жизнь самые продвинутые идеи. ",
    h1: "Основатель студии",
    h2: "Главный разработчик",
    name: "Александр Корицкий",
  },
  {
    label: "Правая рука главного разработчика и во всех смыслах самая прекрасная часть команды",
    h1: "Второй разработчик",
    h2: "Back end разработчик",
    name: "Мария Асташкин",
  },
  {
    label: "Главное контактное лицо студии. Решает любые организационные вопросы.",
    h1: "Бизнес-менеджер",
    h2: "Главный соискатель проектов",
    name: "Максим Евенко ",
  },
  {
    label: "Самый творческий член команды. Создает визуальный облик вашего продукта",
    h1: "Дизайнер и front-end разработчик",
    h2: "Мобильный разработчик",
    name: "Михаил Егоров",
  },
];
